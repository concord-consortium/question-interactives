...
const generateQuestionMap = async () => {
  process.stdout.write("Building question map: ");

  try {
    const data = fs.readFileSync('./question-map.json', {encoding:'utf8', flag:'r'});
    const questionMapFromDisk = JSON.parse(data);
    return { questionMap: questionMapFromDisk, processedActivitiesCount: -1 };
  } catch(e) {
    // continue;
  }
    ...
    await Promise.all(resourceBatchDocs.map(async (resourceDoc) => {
      ...
      try {
        ...
        questions.forEach((question: any) => {
          ...
          const questionKey = `${resource.url}-${sourceQuestionId}`;
          // This might happen when one sequence has one activity added multiple times. But then all the questions
          // should be identical.
          if (questionMap[questionKey] && !deepEqual(safeQuestion, questionMap[questionKey])) {
            writeToFile(errorFile, JSON.stringify({
              type: "question-diff",
              question1: safeQuestion,
              question2: questionMap[questionKey],
            }, null, 2));
          }

          questionMap[questionKey] = safeQuestion;
        });

        if (processedActivitiesCount % 100 === 0) {
  ...
  fs.writeFileSync('./question-map.json', JSON.stringify(questionMap));

  return { questionMap, processedActivitiesCount };
};
...
const executeScript = async () => {
  ...
  const { questionMap, processedActivitiesCount } = await generateQuestionMap();

  let copiedAnswersCount = 0;

  const answersRef = firestore.collection(`sources/${oldSourceKey}/answers`);

  const answersTraverser = createTraverser(answersRef, {
  ...
  await answersTraverser.traverse(async (answerBatchDocs) => {
    ...
    await Promise.all(answerBatchDocs.map(async (answerDoc) => {
      ...
      try {
        ...
        const question = questionMap[`${answer.resource_url}-${answer.question_id}`];
        if (!question) {
          // Nothing to do, question have not been converted.
          return;
        }
  ...
  console.log(`\n\nThe script uses approximately ${used.toFixed(2)} MB of memory`);
  console.log(`Elapsed time: ${durationInS.toFixed(2)} seconds`);
  console.log(`Processed ${processedActivitiesCount} activities, ${activitySpeed.toFixed(2)} activities per second`);
  console.log(`QuestionMap size: ${Object.keys(questionMap).length}`);
