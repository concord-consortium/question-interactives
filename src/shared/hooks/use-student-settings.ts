...
import { IJwtResponse } from "@concord-consortium/lara-interactive-api";
...
const getStudentInfoAndSignInToFirestore = async () => {
  if (!triedToGetStudentInfo) {
    try {
      const firebaseJwt: IJwtResponse = await getFirebaseJWT({ firebaseApp: FIREBASE_APP });
      await signInWithToken(firebaseJwt.token);
      studentInfo = {
        source: parseUrl(firebaseJwt.claims.domain).hostname,
...
export const useStudentSettings = () => {
  ...
  useEffect(() => {
    getStudentInfoAndSignInToFirestore()
      .then(info => {
        if (!info) {
          // Nothing to do, user is not a student running activity from Portal.
          return;
        }
        watchStudentSettings(info, (newSettings: IStudentSettings) => {
