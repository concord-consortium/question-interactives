...
- Cleanup of historical `interactiveState.recordings[]` entries that have `startedAt` set but no `objectId` (orphans left by silent failures BEFORE this fix shipped). With the saving-state visual indicator from Phase 2, those entries now render with the spinner overlay and a `"Recording N - saving..."` aria-label, and the `handleSelectRecording` guard makes them non-selectable — so users on the upgrade path may have one or more wedged "saving..." tiles they can't delete via the UI. The fix surface for this is broader than QI-163 (e.g., a per-mount sweep that filters such entries out of `recordings`/`interactiveState`); it's deferred to a separate ticket. Note that `objectId`-bearing orphans (where `readMetadata` returns `undefined`) ARE handled by the broken-history detection in this fix.
