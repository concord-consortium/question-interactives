...
1. **CODAP form**: Used when `?authoring=codap` is present (after trimming and lowercasing; empty values are treated as “not provided”), OR when the wrapped URL is auto-detected as a CODAP URL
...
- `authoring` - Optional parameter to override authoring form selection
  ...
  - Empty/whitespace/flag-only values (e.g., `?authoring`, `?authoring=`, `?authoring=   `) are treated as “not provided” (auto-detect)
  - Unknown non-empty values (e.g., `?authoring=foo`) fall back to auto-detect and should log a warning in the console
...
# Empty authoring param behaves like not provided (auto-detect)
?wrappedInteractive=https://example.com&authoring=
```
...
1. If `authoring` is a non-empty value, normalize via `trim().toLowerCase()` and look up a matching config
  - `?authoring=codap` → Always use CODAP config (explicit override)
  - Unknown non-empty value (e.g., `?authoring=foo`) → log a warning and fall back to auto-detect
2. If `authoring` is not provided (or is empty/whitespace), use wrapped URL detection:
  - Wrapped URL detected as CODAP → Use CODAP config (auto-detect)
  - Everything else → Use generic config (fallback)
...
export const codapSchema: RJSFSchema = {
  ...
  properties: {
    ...
    displayDataVisibilityToggles: {
      ...
      description: "Adds to graphs an option to toggle the visibility of cases based on the leftmost attribute in the dataset being graphed"
    },
    ...
    lockComponents: {
      ...
      description: "Locks position of components and prevents the user from moving them around the canvas. Only active when 'Remove toolbars & background grid' is checked."
    },
    ...
    advancedOptions: {
      ...
      properties: {
        ...
        enableDiOverride: {
          ...
          description: "Requires a string value that is a substring of the existing plugin URL to be replaced. Only active when 'di' is checked."
        },
        diOverrideValue: {
          title: "value",
          type: "string"
        ...
        enableGuideIndex: {
          ...
          description: "Requires an integer which specifies which guide page to be displayed upon load; default is 0 (first page)"
        },
        guideIndexValue: {
          title: "value",
          type: "integer",
  ...
  computeUiSchema: (formData: any, baseUiSchema: any) => {
    ...
    return {
      ...
      advancedOptions: {
        ...
        enableDiOverride: {
          ...baseUiSchema.advancedOptions?.enableDiOverride,
...
Transform the App component to use the standard `BaseQuestionApp` pattern (consistent with other packages like `bar-graph`):
...
// BaseQuestionApp passes standard props (authoredState, setAuthoredState)
// but Authoring needs extra props for config selection
const AuthoringWrapper: React.FC<IAuthoringComponentProps<IAuthoredState>> = ({
  ...
  // Adapter: setAuthoredState expects an update function (UpdateFunc<T>),
  // but Authoring passes the new state directly. Wrap it to match the expected signature.
  ...
  return (
    <Authoring
      ...
      originalWrappedUrl={wrappedInteractive || undefined}
    />
  );
};

// Use BaseQuestionApp for consistent pattern with other question interactives
// BaseQuestionApp handles:
// - Mode detection (authoring vs runtime) via LARA init message
// - State management via LARA hooks
// - Passing appropriate props to Authoring/Runtime components
export const App = () => (
...
Modify the Runtime component to accept props from `BaseQuestionApp` using the standard `IRuntimeQuestionComponentProps` interface:
...
// Runtime receives props from BaseQuestionApp via IRuntimeQuestionComponentProps
// This pattern is consistent with other question interactives (bar-graph, etc.)
...
│  │ https%3A%2F%2Fcfm-shared.concord.org%2FkQt77dsG4A1NLyTBqpLN%2F      │    │
│  │ file.json                                                           │    │
│  │                                                                     │    │
│  │                                                                     │    │
│  │                                                                     │    │
│  │                                                                     │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│  Paste a share link displayed in the CODAP Share dialog                     │
│                                                                             │
│  ─────────────────────────────────────────────────────────────────────────  │
│                                                                             │
│  CODAP Options                                                              │
...
│    Adds to graphs an option to toggle the visibility of cases based on the  │
│    leftmost attribute in the dataset being graphed                          │
│                                                                             │
│  ☑ Display all visible components within the bounds of the visible CODAP    │
│    canvas (no scrollbars) (inbounds=true)                                   │
│    Keeps components displayed inside the browser window. If the window/     │
│    browser is resized components will move to stay within bounds of the     │
│    CODAP canvas. Users will not be able to place any components so that     │
│    they would extend beyond the boundaries of the currently visible canvas. │
│    (Note, can result in components overlapping or being moved from the      │
│    original layout if the document is opened on a smaller screen than       │
...
│      around the canvas. Only active when "Remove toolbars & background      │
│      grid" is checked.                                                      │
│                                                                             │
│  ─────────────────────────────────────────────────────────────────────────  │
│                                                                             │
...
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │                                                                       │  │
│  │  ☐ Load a plugin from a url (di=url)                                  │  │
│  │    Requires a string specifying the URL of a plugin to load           │  │
│  │    Plugin URL ┌─────────────────────────────────────────────────┐     │  │
│  │               │                                          [disabled]   │  │
│  │               └─────────────────────────────────────────────────┘     │  │
│  │                                                                       │  │
│  │    ☐ Replace the contents of an existing plugin or webview            │  │
│  │      (di-override=string)                                             │  │
│  │      Requires a string value that is a substring of the existing      │  │
│  │      plugin URL to be replaced. Only active when "di" is checked.     │  │
│  │      value    ┌─────────────────────────────────────────────────┐     │  │
│  │               │                                          [disabled]   │  │
│  │               └─────────────────────────────────────────────────┘     │  │
│  │                                                                       │  │
│  │  ☐ Display a specific guide page (guideIndex=int)                     │  │
│  │    Requires an integer which specifies which guide page to be         │  │
│  │    displayed upon load; default is 0 (first page)                     │  │
│  │    value      ┌─────────────────────────────────────────────────┐     │  │
│  │               │ 0                                        [disabled]   │  │
│  │               └─────────────────────────────────────────────────┘     │  │
│  │                                                                       │  │
...
│  │  Generated URL                                                        │  │
│  │    ┌─────────────────────────────────────────────────────────────┐    │  │
│  │    │ https://codap3.concord.org/?interactiveApi&documentId=      │    │  │
│  │    │ https%3A%2F%2Fcfm-shared.concord.org%2FkQt77dsG4A1NLyTBqpLN │    │  │
│  │    │ %2Ffile.json&app=is&inbounds=true              [read-only]  │    │  │
│  │    └─────────────────────────────────────────────────────────────┘    │  │
│  │    The final URL that will be used for this CODAP interactive         │  │
│  │    (read-only)                                                        │  │
│  │                                                                       │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
...
1. **Paste source URL** into "CODAP Source Document URL" field. Accepts any of these formats:
   ...
   - Iframe embed from CODAP Share dialog: `<iframe ... src="https://codap.concord.org/...#shared=..."></iframe>`
...
2. **Configure CODAP Options** via checkboxes:
   ...
   - Display all visible components within the bounds of the visible CODAP canvas (no scrollbars) (default: unchecked)
   - Remove toolbars & background grid (default: unchecked)
     - Lock components (default: unchecked) - only active when above is checked
...
Implementation is organized into phases with clear milestones. Each phase builds on the previous and produces a testable deliverable.

### Phase 1: Foundation
**Steps 1-3** | *Types, configuration system, utilities*
...
**Milestone:** Type system and configuration infrastructure in place. URL parsing/building can be unit tested.

---

### Phase 2: Authoring Interface
**Steps 4-6** | *Authoring UI and app integration*
...
6. **Update app.tsx** *(Proposed Changes 1 and 6)*
   ...
   - Use LARA hooks (`useAuthoredState`, `useInitMessage`) for state persistence and mode detection

**Milestone:** Authoring mode fully functional. Authors can configure interactives and save state.

---

### Phase 3: Runtime Integration
**Steps 7-8** | *Runtime behavior from authored state*
...
**Milestone:** Complete authoring-to-runtime flow. Authored configuration affects runtime behavior.
...
9. **Testing**
   ...
   - **RJSF Upgrade Regression Test** (known fragile integration):
     ...
     - Check that `__severity` is still accessible via `FieldErrorProps.errorSchema` in `FieldErrorTemplate`
...
**Milestone:** Feature complete with test coverage and documentation.

---

### Phase 5: Form Refinements *(Future Work)*
*Form layout updates based on product owner feedback*

11. **Form layout and UX refinements**
    - Gather product owner feedback on initial implementation
    - Update form field ordering, grouping, or labels as needed
    - Adjust which options are in "Advanced" vs main sections
    - Refine validation messages and help text
    - Iterate on UI based on author usability testing

**Milestone:** Form UX refined based on real-world feedback from product owners and curriculum authors.
...
- Authoring UI appears only when LARA opens the interactive in authoring mode (`initMessage.mode === "authoring"`)
- In authoring mode, `?authoring=<type>` and URL pattern matching only select which authoring form/config is used
...
### Collapsible Sections (Custom ObjectFieldTemplate)

RJSF's `@rjsf/core` package doesn't natively support collapsible object fields - that's typically a feature of theme-specific packages like `@rjsf/bootstrap-4` or `@rjsf/mui`. To enable collapsible sections (like "Advanced CODAP Options") without adding additional dependencies, we use a custom `ObjectFieldTemplate`.
...
**Implementation:** The custom `ObjectFieldTemplate` in `authoring.tsx`:
- Checks for `ui:options.collapsible` in the uiSchema
- If true, renders the object's content inside native HTML `<details>` and `<summary>` elements
- Uses the object's `title` as the summary header
- Respects `ui:options.collapsed` to set the initial open/closed state

**Benefits of using `<details>`/`<summary>`:**
- Native browser support - no JavaScript required for toggle behavior
- Built-in accessibility - keyboard navigation (Enter/Space to toggle), screen reader announcements
- Maintains open/closed state without React state management
- Consistent behavior across browsers

**Styling:**
- Border and rounded corners around the collapsible section
- Header has subtle background color and hover effect
- Custom arrow indicator (▶) that rotates when expanded
- Smooth CSS transition for the rotation animation

This approach stays entirely within the full-screen package and leverages RJSF's existing template system without modifying external packages.
...
- [ ] CODAP Source Document URL field accepts all supported URL formats:
  ...
  - [ ] Iframe embed: `<iframe ... src="https://codap.concord.org/...#shared=<encoded-url>"></iframe>`
...
| `<iframe ... src="https://codap.concord.org/...#shared=https%3A..."></iframe>` | iframe-embed | `https://codap.concord.org/...` | decoded URL |
...
- [ ] "Display all visible components within the bounds of the visible CODAP canvas (no scrollbars)" checkbox (default: unchecked) adds `inbounds=true` when checked
- [ ] "Remove toolbars & background grid" checkbox (default: unchecked) adds `embeddedMode=yes`
  - [ ] "Lock components" sub-checkbox (default: unchecked) adds `componentMode=yes` - only active when parent is checked

### Advanced CODAP Options
- [ ] Advanced options section is collapsed by default
- [ ] "di" checkbox enables Plugin URL field, adds `di=<url>` parameter
  - [ ] "di-override" sub-checkbox enables value field, adds `di-override=<value>` parameter - only active when "di" is checked
...
- [ ] Specifically test: `__severity` accessible via `FieldErrorProps.errorSchema` in custom `FieldErrorTemplate`
