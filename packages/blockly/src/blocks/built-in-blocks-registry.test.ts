import { BUILT_IN_BLOCKS } from "./built-in-blocks-registry";

describe("built-in-blocks-registry", () => {
  describe("BUILT_IN_BLOCKS", () => {
    it("should contain the expected built-in Blockly blocks", () => {
      expect(BUILT_IN_BLOCKS).toHaveLength(3);
      
      const blockIds = BUILT_IN_BLOCKS.map(block => block.id);
      expect(blockIds).toContain("controls_if");
      expect(blockIds).toContain("logic_operation");
      expect(blockIds).toContain("logic_negate");
      
      const blockNames = BUILT_IN_BLOCKS.map(block => block.name);
