...
This documentation will reference concepts from Atomic Agents and to a less extent Atomic Agents Vis.
...
The simulation code has its own text area when editing a blockly interactive. All of the code included here will be sent as-is to the agent simulation interactive. You could put the same code you would put in an agent simulation interactive's simulation code area here, and the agent simulation interactive would work in exactly the same way.
...
This block defines the `setup` function. In order for this block to do anything, include `setup();` at the bottom of the simulation code.
...
#### On Mouse Click (Under Construction)
...
- `callback` is a function like `(agent) => { [statements] }`
  - `agent` is the newly created agent (the callback will be called with every agent created)
...
- If provided, `callback` should be called with each new actor as an argument.
...
Custom action blocks give authors a lot of flexibility to create blocks that can do a wide range of things that a simulation might want. Many different types of fields can be added to action blocks. And action blocks require custom code to be specified by an author.
