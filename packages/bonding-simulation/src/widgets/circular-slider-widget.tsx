...
export const CircularSliderWidget = observer(function CircularSliderWidget(props: IWidgetComponentProps<CircularSliderWidgetData>) {
  const { data, globalKey, globals } = props;
  const isCompletedRecording = false;
  const inRecordingMode = false;
  ...
  const value = (globals.get(globalKey) ?? min) as number;
  ...
  const handleValueChange = useCallback(
    (newValue: number) => {
      ...
      globals.set(globalKey, snappedValue);
    },
    [globals, globalKey, min, max, step]
  );
  ...
  const globalValue = globals.get(globalKey);
